...
test.describe('Data Validation Tests', () => {
  test('3.1 User creation — required field checks (name, email, accountType) each fail independently', async ({
    ...
    await expect(missingNameResponse).toBeValidationError('name is required');
    ...
    await expect(missingEmailResponse).toBeValidationError('a valid email is required');
    ...
    await expect(missingAccountTypeResponse).toBeValidationError('accountType must be one of: basic, premium');
  });
  ...
  test('3.3 Email regex edge cases reachable only at the API layer', async ({ request }) => {
    ...
    for (const email of invalidEmails) {
      ...
      await expect(response).toBeValidationError('a valid email is required');
    }
  ...
  test('3.4 accountType allow-list is enforced independently of the UI dropdown', async ({ request }) => {
    ...
    await expect(goldResponse).toBeValidationError('accountType must be one of: basic, premium');
    ...
    await expect(emptyStringResponse).toBeValidationError('accountType must be one of: basic, premium');
    ...
    await expect(nullResponse).toBeValidationError('accountType must be one of: basic, premium');
  });
  ...
  test('3.5 Duplicate email is rejected with 409, case-insensitively', async ({ request }) => {
    ...
    await expect(secondResponse).toBeConflictError('email already registered');
    ...
    await expect(casedResponse).toBeConflictError('email already registered');
  });
  ...
  test('3.6 Wrong JS type for string fields is rejected, not coerced', async ({ request }) => {
    ...
    await expect(numericNameResponse).toBeValidationError('name is required');
    ...
    await expect(booleanEmailResponse).toBeValidationError('a valid email is required');
  });
